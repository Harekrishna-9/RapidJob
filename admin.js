...
function previewHtml(d){return `<div class="preview-post"><div class="preview-head"><span class="tag">${esc(d.categoryName)}</span><h1>${esc(d.title||"Post Title")}</h1><p><i class="fa-regular fa-clock"></i> Updated On : ${esc(d.updatedOn||"")}</p></div><p>${br(d.description||"")}</p>${adminJourneyPreview(d)}${(d.customBlocks||[]).map(renderBlock).join("")}</div>`}
...
const JOB_JOURNEY_DEFAULTS=[
  ["notification","Notification Released","fa-bullhorn"],
  ["application","Application Started","fa-file-signature"],
  ["lastDate","Application Last Date","fa-calendar-xmark"],
  ["correction","Correction Window","fa-pen-to-square"],
  ["city","City Intimation","fa-location-dot"],
  ["admitCard","Admit Card","fa-id-card"],
  ["exam","Exam Date","fa-clipboard-question"],
  ["answerKey","Answer Key","fa-key"],
  ["objection","Objection Window","fa-triangle-exclamation"],
  ...
  ["joining","Joining Letter","fa-handshake"],
];
function normalizeJourneyData(p={}){
  const stored=p.jobJourney||{};
  const oldStages=Array.isArray(stored.stages)?stored.stages:[];
  return {
    enabled:Boolean(stored.enabled),
    stages:JOB_JOURNEY_DEFAULTS.map(([key,label,icon],index)=>{
      const s=oldStages.find(x=>x&&x.key===key)||oldStages[index]||{};
      return {key,label:s.label||label,icon:s.icon||icon,status:s.status||"upcoming",date:s.date||"",link:s.link||""};
    })
  };
}
function journeyEditorHtml(p={}){
  const journey=normalizeJourneyData(p);
  return `<section class="job-journey-editor" id="jobJourneyEditor">
    <div class="journey-editor-head">
      <div><span class="journey-kicker"><i class="fa-solid fa-route"></i> Recruitment Tracker</span><h3>Job Journey</h3><p>Isi Job Editor se recruitment ke har stage ko update karein.</p></div>
      <label class="journey-master-toggle"><input id="journeyEnabled" type="checkbox" ${journey.enabled?"checked":""}><span></span><b>Show on Job Page</b></label>
    </div>
    <div class="journey-admin-progress"><div><b>Journey Progress</b><small id="journeyProgressText">0% Completed</small></div><div class="journey-admin-track"><span id="journeyProgressBar"></span></div></div>
    <div class="journey-stage-grid" id="journeyStageGrid">
      ${journey.stages.map((s,i)=>`<div class="journey-stage-row" data-key="${esc(s.key)}" data-icon="${esc(s.icon)}">
        <div class="journey-stage-no">${i+1}</div>
        <div class="journey-stage-name"><i class="fa-solid ${esc(s.icon)}"></i><input class="journey-label" value="${esc(s.label)}" aria-label="Stage name"></div>
        <select class="journey-status" aria-label="Stage status"><option value="upcoming" ${s.status==="upcoming"?"selected":""}>Upcoming</option><option value="current" ${s.status==="current"?"selected":""}>Current</option><option value="completed" ${s.status==="completed"?"selected":""}>Completed</option></select>
        <input class="journey-date" type="date" value="${esc(s.date)}" aria-label="Stage date">
        <input class="journey-link" type="url" value="${esc(s.link)}" placeholder="Optional link" aria-label="Stage link">
      </div>`).join("")}
    </div>
    <div class="journey-editor-note"><i class="fa-solid fa-circle-info"></i> Ek samay par sirf ek stage ko <b>Current</b> rakhein. Completed stages green, current stage yellow aur upcoming stages grey dikhenge.</div>
  </section>`;
}
function bindJourneyEditor(){
  const root=document.getElementById("jobJourneyEditor");if(!root)return;
  const update=()=>{
    const rows=[...root.querySelectorAll(".journey-stage-row")];
    const completed=rows.filter(r=>r.querySelector(".journey-status")?.value==="completed").length;
    const pct=rows.length?Math.round(completed/rows.length*100):0;
    const bar=document.getElementById("journeyProgressBar"),txt=document.getElementById("journeyProgressText");
    if(bar)bar.style.width=pct+"%";if(txt)txt.textContent=pct+"% Completed";
    rows.forEach(r=>{r.dataset.status=r.querySelector(".journey-status")?.value||"upcoming"});
  };
  root.addEventListener("change",e=>{
    if(e.target.classList.contains("journey-status")&&e.target.value==="current"){
      root.querySelectorAll(".journey-status").forEach(x=>{if(x!==e.target&&x.value==="current")x.value="upcoming"});
    }
    update();
  });
  update();
...
function collectJourneyData(){
  const root=document.getElementById("jobJourneyEditor");
  if(!root)return {enabled:false,stages:[]};
  return {enabled:Boolean(document.getElementById("journeyEnabled")?.checked),stages:[...root.querySelectorAll(".journey-stage-row")].map(r=>({
    key:r.dataset.key||"",icon:r.dataset.icon||"fa-circle",label:r.querySelector(".journey-label")?.value.trim()||"Stage",
    status:r.querySelector(".journey-status")?.value||"upcoming",date:r.querySelector(".journey-date")?.value||"",link:r.querySelector(".journey-link")?.value.trim()||""
  }))};
}
function adminJourneyPreview(d){
  const j=d.jobJourney;if(!j?.enabled||!Array.isArray(j.stages))return "";
  const current=j.stages.find(s=>s.status==="current");
  const done=j.stages.filter(s=>s.status==="completed").length;
  const pct=Math.round(done/j.stages.length*100)||0;
  return `<div class="admin-journey-preview"><h2>Job Journey</h2><p><b>Current Stage:</b> ${esc(current?.label||"Not selected")} &nbsp; • &nbsp; <b>${pct}% Completed</b></p><div class="admin-journey-preview-list">${j.stages.map(s=>`<span class="${esc(s.status)}">${esc(s.label)}</span>`).join("")}</div></div>`;
}
...
openPostForm=function(id=null){
  ...
  journeyWrap.innerHTML=journeyEditorHtml(p);
  sticky.parentNode.insertBefore(journeyWrap.firstElementChild,sticky);
...
collectPostData=function(statusOverride=null){
  ...
  Object.assign(d,{
    ...
    autoUnpublish:chk("autoUnpublish"),jobJourney:collectJourneyData()
  });
