...
function initAdminLogin(){
  ...
  if(localStorage.getItem(ADMIN_SESSION_KEY) === "yes"){
    unlockAdmin();
  }else{
    lockAdmin();
  }
  ...
  if(toggleSecret){
    toggleSecret.onclick = () => {
      ...
      toggleSecret.innerHTML = secretInput.type === "password" ? '<i class="fa-solid fa-eye"></i>' : '<i class="fa-solid fa-eye-slash"></i>';
    };
  }
  ...
  function doLogin(){
    ...
    if(!email || !email.includes("@")){
      if(loginError) loginError.textContent = "Please enter valid email address.";
      return;
    }

    if(code !== ADMIN_SECRET_CODE){
      if(loginError) loginError.textContent = "Wrong secret code.";
  ...
  if(secretInput) secretInput.addEventListener("keydown", e => { if(e.key === "Enter") doLogin(); });
  if(emailInput) emailInput.addEventListener("keydown", e => { if(e.key === "Enter") secretInput?.focus(); });
...
firebase.initializeApp(firebaseConfig);const db=firebase.database();const $=q=>document.querySelector(q);let posts=[],messages=[],current="dashboard",editId=null;
...
function br(v){return esc(v).replaceAll("\n","<br>")}function catName(v){return (categories.find(c=>c[0]===v)||["",v||"Current Job"])[1]}
function page(t,s,b){return `<div class="hero"><span class="tag">Rapid Job CMS</span><h1>${t}</h1><p>${s}</p></div>${b}`}
function dashboard(){const pub=posts.filter(p=>p.status!=="Draft").length,draft=posts.filter(p=>p.status==="Draft").length;return page("Dashboard","Aapka visual job portal control center.",`<div class="stats"><div class="stat"><p>Total Posts</p><h2>${posts.length}</h2></div><div class="stat"><p>Published</p><h2>${pub}</h2></div><div class="stat"><p>Draft</p><h2>${draft}</h2></div><div class="stat"><p>Messages</p><h2>${messages.length}</h2></div></div><div class="panel"><div class="panel-head"><h2>Recent Posts</h2><button class="btn" onclick="openPostForm()">+ Add Post</button></div>${postTable(posts.slice(0,8))}</div>`)}
function postTable(list){return `<div class="table-wrap"><table class="table"><thead><tr><th>Title</th><th>Category</th><th>Status</th><th>Date</th><th>Action</th></tr></thead><tbody>${list.map(p=>`<tr><td>${esc(p.title||"")}</td><td>${esc(catName(p.type))}</td><td><span class="badge ${p.status==='Draft'?'draft':''}">${esc(p.status||"Published")}</span></td><td>${esc(p.date||p.updatedOn||"")}</td><td><button class="mini view" onclick="previewSavedPost('${p.id}')">Preview</button><button class="mini edit" onclick="openPostForm('${p.id}')">Edit</button><button class="mini del" onclick="deletePost('${p.id}')">Delete</button></td></tr>`).join("")||'<tr><td colspan="5">No posts found.</td></tr>'}</tbody></table></div>`}
function postsPage(){return page("Post Management","Yahan se aap apne mann ke anusaar table, link, button, notice, card, HTML kuch bhi add kar sakte ho.",`<div class="panel"><div class="panel-head"><h2>All Posts</h2><button class="btn" onclick="openPostForm()">+ Add New Post</button></div>${postTable(posts)}</div>`)}
...
function loadFastTemplate(){if(confirm("FastJob template add karna hai? Current custom blocks clear ho jayenge.")){$("#customBlocks").innerHTML="";demo.customBlocks.forEach(b=>addBlock(b.style,b))}}
...
function alertPopup(m){openAdminModal(`<div class="modal-body"><h2>Success</h2><p>${esc(m)}</p><button class="btn" onclick="closeAdminModal()">OK</button></div>`)}function previewDraft(){let d=collectPostData();if(!d.title)return alert("Title required");openAdminModal(`<div class="modal-title-row"><h2>Post Preview Before Publish</h2><button class="btn green" onclick="closeAdminModal();savePost('Published')">Publish Now</button></div>${previewHtml(d)}`)}function previewSavedPost(id){let d=posts.find(p=>p.id===id);if(!d)return alert("Post not found");openAdminModal(`<div class="modal-title-row"><h2>${esc(d.title)}</h2><button class="btn" onclick="window.open('job-detail.html?id=${id}','_blank')">Open Page</button></div>${previewHtml(d)}`)}
...
function messagesPage(){return page("Contact Messages","Contact page messages.",`<div class="panel">${messages.map(m=>`<div class="message-card"><b>${esc(m.name||"No Name")}</b><p>${esc(m.message||"")}</p></div>`).join("")||"No messages yet."}</div>`)}function breaking(){return page("Breaking News","Ticker update manage karein.",`<div class="panel"><textarea id="breakingText" style="width:100%;min-height:140px"></textarea><br><br><button class="btn" onclick="db.ref('settings/breakingNews').set($('#breakingText').value);alertPopup('Saved')">Save</button></div>`)}function media(){return page("Media Manager","Media links store karein.",`<div class="panel"><p>Use direct image/PDF URL in visual builder.</p></div>`)}function blogEditor(){
  return page("Blog / Article Editor","Blog, article, yojana, document download ke liye custom editor.",`
    <div class="panel">
      <div class="panel-head">
        <h2>Blog Article Builder</h2>
        <button class="btn" onclick="openPostForm()">+ Create Blog Post</button>
      </div>

      <div class="cms-card">
        <h3>📰 Blog Format</h3>
        <p>Blog ke liye Post Management me category <b>Latest Blog</b> select karo.</p>
        <p>Phir Visual Builder me Image, Table, Notice, Button, FAQ aur Links add kar sakte ho.</p>
      </div>
    </div>
  `);
