...
function normalizeBlogBlockOrder(blocks=[]){
  ...
  const isEligibility=title=>{
    const t=String(title||"").toLowerCase();
    return t.includes("पात्रता") || t.includes("eligibility");
  };

  const isHelpDesk=title=>{
    const t=String(title||"").toLowerCase();
    return t.includes("help desk") || t.includes("helpline") || t.includes("सहायता");
  };

  const eligibility=list.find(isEligibility);
  const helpDesk=list.find(isHelpDesk);

  if(eligibility && helpDesk && Number(helpDesk.order)<=Number(eligibility.order)){
    helpDesk.order=Number(eligibility.order)+1;
  }

  return list.sort((a,b)=>Number(a.order||50)-Number(b.order||50));
}
