...
function openBlogForm(id=null){
  ...
  (b.blocks&&b.blocks.length?b.blocks:[
  ...
  ]).forEach(x=>addBlogBlock(x.style,x));
}
...
function collectBlogData(status){
  ...
  })).filter(b=>b.title||b.content).sort((a,b)=>a.order-b.order);
  ...
  return {
    ...
    blog:{
      ...
      summary:get("blogSummary"),importantLinks,blocks
    },
    importantLinks,
    customBlocks:blocks.map(x=>({
      title:x.title,
...
function blogPreviewHtml(d){
  ...
  return `<div class="preview-post">
    ...
    <div class="blog-preview-body">
      ...
      ${(b.blocks||[]).map(renderBlogBlock).join("")}
    </div>
  </div>`
}
