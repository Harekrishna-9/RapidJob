...
function form(type='job',editIndex=''){
 ...
 return `<div class="cms-form-wrap"><div class="cms-head"><h2>Fast Job Style Post Box</h2><p>Example text box ke andar diya hai. Tum bas apna details change karke Save Post dabao.</p></div><div class="cms-grid"><input id="postTitle" placeholder="Title / Job Name" value="${tmp.title}"><select id="postType" onchange="openPostForm(this.value)">${optionsHtml(type)}</select><input id="postDate" type="date"><select id="postStatus"><option>Published</option><option>Draft</option><option>Pending</option></select><input id="postState" placeholder="State / All India" value="All India"><input id="postLastDate" placeholder="Last Date / Released / New" value="Check details"><input id="applyLink" placeholder="Apply Online Link"><input id="officialLink" placeholder="Official Website Link"><input id="pdfLink" placeholder="Notification PDF Link"><select id="postTrending"><option value="false">Trending: No</option><option value="true">Trending: Yes</option></select></div><label class="cms-label">Short Description</label><textarea id="postDetails" placeholder="Short details / description">${tmp.short}</textarea><div class="cms-section"><h3>▣ Important Dates</h3><textarea id="importantDates">${tmp.dates}</textarea></div><div class="cms-section"><h3>▣ Application Fee</h3><textarea id="applicationFee">${tmp.fee}</textarea></div><div class="cms-section"><h3>▣ Age Limit</h3><textarea id="ageLimit">${tmp.age}</textarea></div><div class="cms-section"><h3>▣ Vacancy / Total Post Details</h3><textarea id="vacancyDetails">${tmp.vacancy}</textarea></div><div class="cms-section"><h3>▣ Selection Process / Eligibility</h3><textarea id="selectionProcess">${tmp.selection}</textarea></div><div class="cms-actions"><button class="btn" onclick="savePost(${editIndex===''?'null':editIndex})">Save Post</button><button class="btn light" onclick="openPostForm($('#postType').value)">Reset Template</button><button class="btn light" onclick="render('content')">Back</button></div></div>`
}
...
function editPost(i){let p=posts[i];$('#content').innerHTML=page('Edit Post','Post details update karein.',form(p.type,i),'Edit');$('#postTitle').value=p.title||'';$('#postDate').value=p.date||'';$('#postStatus').value=p.status||'Published';$('#postState').value=p.state||'All India';$('#postLastDate').value=p.lastDate||'';$('#applyLink').value=p.applyLink||'';$('#officialLink').value=p.officialLink||'';$('#pdfLink').value=p.pdfLink||'';$('#postTrending').value=String(!!p.trending);$('#postDetails').value=p.details||'';$('#importantDates').value=p.importantDates||'';$('#applicationFee').value=p.applicationFee||'';$('#ageLimit').value=p.ageLimit||'';$('#vacancyDetails').value=p.vacancyDetails||'';$('#selectionProcess').value=p.selectionProcess||''}
...
function openPostForm(type='job'){$('#content').innerHTML=page('Add New Post','Fast Job style content add karein.',form(type),'New Post')}
