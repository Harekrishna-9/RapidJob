...
function loadData(){
 ...
 db.ref('posts').on('value',s=>{
  ...
  Object.entries(data).forEach(([id,p])=>{
    ...
    if(p.title || p.postTitle || p.name || p.details || p.description){
      arr.push({id,...p,type:normalizeType(p.type||p.category)});
    }else{
      Object.entries(p).forEach(([childId,child])=>{
        if(child && typeof child==='object'){
          arr.push({id:childId,category:id,...child,type:normalizeType(child.type||child.category||id)});
        }
      });
    }
  });
  POSTS=arr.length?arr.sort((a,b)=>(b.time||b.createdAt||0)-(a.time||a.createdAt||0)):fallbackPosts;
  render();
...
function render(){
 ...
 if($('topCards')) $('topCards').innerHTML=POSTS.slice(0,8).map(p=>`<div class="quick-card"><a href="job-detail.html?id=${p.id}${p.category?'&category='+p.category:''}">🔥 ${p.title}</a></div>`).join('');
...
function mini(p){return `<a class="mini-item" href="job-detail.html?id=${p.id}${p.category?'&category='+p.category:''}">${p.title} ${p.lastDate&&p.lastDate.toLowerCase().includes('out')?'<small>OUT</small>':''}</a>`}
function miniEmpty(t){return `<span class="mini-item">${t}</span>`}
function postCard(p){return `<div class="post"><span class="tag">${label(p.type)}</span><h3>${p.title}</h3><p><b>State:</b> ${p.state||'Central'} | <b>Date:</b> ${p.lastDate||'Check details'}</p><p>${(p.details||'Click view details for full information.').slice(0,150)}...</p><a href="job-detail.html?id=${p.id}${p.category?'&category='+p.category:''}">View Details</a></div>`}
...
function updateSlider(){const arr=POSTS.length?POSTS:fallbackPosts;slideIndex=(slideIndex+arr.length)%arr.length;const p=arr[slideIndex];if($('slideLink')){$('slideLink').href=`job-detail.html?id=${p.id}${p.category?'&category='+p.category:''}`;$('slideLink').innerHTML=`<span>${p.trending?'New':'Released'}</span><b>🔥 ${p.title}</b>`}}
