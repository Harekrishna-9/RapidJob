...
function listBy(type){return POSTS.filter(p=>String(p.type||'').toLowerCase()===type)}
function listByMany(types){const set=types.map(t=>String(t).toLowerCase());return POSTS.filter(p=>set.includes(String(p.type||'').toLowerCase()))}
...
function label(t){return {job:'Current Job',admit:'Admit Card',result:'Result',answer:'Answer Key',syllabus:'Syllabus',admission:'Admission',scholarship:'Scholarship',blog:'Latest Blog',latestblog:'Latest Blog',document:'Document'}[String(t).toLowerCase()]||'Update'}
