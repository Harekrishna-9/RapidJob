...
function render(){
 ...
 const oldState=$('stateFilter').value||'All';
 const states=[...new Set(POSTS.map(p=>p.state||'Central'))];
 $('stateFilter').innerHTML='<option value="All">All States</option>'+states.map(s=>`<option ${oldState===s?'selected':''}>${s}</option>`).join('');
 const st=$('stateFilter').value||'All';
