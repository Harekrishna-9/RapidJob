...
function save(){const data={};fields.forEach(f=>{if($(f))data[f]=$(f).value});localStorage.setItem('resumeProClean',JSON.stringify(data));updateStrength();alert('Resume saved successfully!')}
...
$('resetBtn').onclick=()=>{if(confirm('Clear all details?')){localStorage.removeItem('resumeProClean');fields.forEach(f=>{if($(f))$(f).value=''});$('template').value='executive';update();}};
