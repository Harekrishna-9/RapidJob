...
function renderDetail(p){
  ...
  detailBox.innerHTML=`
    ...
    <div class="job-card">
      ...
      <div class="two-col"><div class="mini-panel"><h3>🏆 Selection Process</h3><ul>${list(p.selectionProcess)}</ul></div><div class="mini-panel"><h3>▣ Exam Pattern (CBT)</h3>${examTable(p)}</div><div class="mini-panel"><h3>₹ Salary / Pay Scale</h3><p>${br(val(p,'salary','Check notification for salary details.'))}</p></div><div class="mini-panel"><h3>📦 How to Apply</h3><ol>${list(p.howToApply).replaceAll('<li>','<li>')}</ol></div><div class="mini-panel links-panel"><h3>🔗 Important Links</h3><p>➤ <a href="${val(p,'applyLink','#')}" target="_blank">Apply Online</a><br>➤ <a href="${val(p,'pdfLink','#')}" target="_blank">Official Notification</a><br>➤ <a href="index.html#syllabus">Syllabus</a><br>➤ <a href="${val(p,'officialLink','#')}" target="_blank">Official Website</a></p></div><div class="mini-panel faq-panel"><h3>❔ FAQ</h3>${faqHtml(p)}</div></div>
