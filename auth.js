...
function initDashboard(){
  ...
  requireUser(async(user,p)=>{
    ...
    if(welcome)welcome.textContent="Welcome, "+(p.name||user.displayName||"User");

    const statSaved=$("statSaved");
    const statBookmarks=$("statBookmarks");
    const statApplied=$("statApplied");
    const statRecent=$("statRecent");
    const statTests=$("statTests");
    const statDownloads=$("statDownloads");
    const statNotifications=$("statNotifications");

    if(statSaved)statSaved.textContent=countObject(p.savedJobs);
    if(statBookmarks)statBookmarks.textContent=countObject(p.bookmarks);
    if(statApplied)statApplied.textContent=countObject(p.appliedJobs);
    if(statRecent)statRecent.textContent=countObject(p.recentlyViewed);
    if(statTests)statTests.textContent=countObject(p.mockHistory)+countObject(p.quizHistory);
    if(statDownloads)statDownloads.textContent=countObject(p.downloadHistory);
    ...
    if(statNotifications)statNotifications.textContent=notices.length;
