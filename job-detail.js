...
function formatJourneyDate(v){if(!plain(v))return"";try{return new Date(`${v}T00:00:00`).toLocaleDateString("en-IN",{day:"2-digit",month:"short",year:"numeric"})}catch(e){return v}}
function jobJourneyHtml(p){const j=p?.jobJourney;if(!j?.enabled||!Array.isArray(j.steps)||!j.steps.length)return"";const steps=j.steps.filter(x=>x&&x.label);if(!steps.length)return"";const completed=steps.filter(x=>x.status==="completed").length;const current=steps.find(x=>x.status==="current");const progress=Math.max(0,Math.min(100,Math.round(((completed+(current?0.5:0))/steps.length)*100)));return `<section class="rj-job-journey"><div class="rj-journey-head"><div><span class="rj-journey-kicker"><i class="fa-solid fa-route"></i> LIVE RECRUITMENT TRACKER</span><h2>Job Journey</h2><p>Notification se joining tak recruitment ki current progress.</p></div><div class="rj-journey-percent"><strong>${progress}%</strong><span>Completed</span></div></div><div class="rj-journey-progress"><span style="width:${progress}%"></span></div>${current?`<div class="rj-current-stage"><i class="fa-solid fa-location-dot"></i><div><small>Current Stage</small><b>${esc(current.label)}</b>${current.date?`<span>${esc(formatJourneyDate(current.date))}</span>`:""}</div></div>`:""}<div class="rj-journey-timeline">${steps.map((x,i)=>{const status=["completed","current","upcoming"].includes(x.status)?x.status:"upcoming";const icon=status==="completed"?"fa-check":status==="current"?"fa-bolt":"fa-clock";const inner=`<div class="rj-journey-icon"><i class="fa-solid ${icon}"></i></div><div class="rj-journey-copy"><div><span>Step ${i+1}</span><b>${esc(x.label)}</b></div>${x.date?`<time><i class="fa-regular fa-calendar"></i> ${esc(formatJourneyDate(x.date))}</time>`:"<time>Date to be announced</time>"}</div>`;return x.link?`<a class="rj-journey-step ${status}" href="${esc(x.link)}" target="_blank" rel="noopener">${inner}<i class="fa-solid fa-arrow-up-right-from-square rj-journey-open"></i></a>`:`<div class="rj-journey-step ${status}">${inner}</div>`}).join("")}</div></section>`}
...
function scholarshipImportantLinks(p){
  const list=Array.isArray(p.importantLinks)?p.importantLinks:[];
  return list.filter(x=>x&&x.enabled!==false&&plain(x.title)).sort((a,b)=>Number(a.order||50)-Number(b.order||50));
}
function renderScholarshipDetail(p){
  injectFastJobStyles();
  const title=field(p,["title","postTitle","name"],"Scholarship Update");
  const updated=field(p,["updatedOn","updatedAt","timeText","publishedAt"],todayText());
  const desc=field(p,["description","details","content","summary"],"Please check official notification before applying.");
  const links=scholarshipImportantLinks(p);
  const apply=(links.find(x=>/apply|login/i.test(String(x.buttonType||x.title||'')))||{}).url||"#";
  const official=(links.find(x=>/official|website/i.test(String(x.title||x.buttonType||'')))||{}).url||"#";
  const docs=lines(field(p,["documents","requiredDocuments"],""));
  detailBox.innerHTML=`
    <section class="fj-top-card"><span class="fj-tag">Scholarship</span><h1 class="fj-title">${esc(title)}</h1><div class="fj-updated">◷ Updated On : ${esc(updated)}</div></section>
    <div class="fj-desc">${nl2br(desc)}</div>
    <div class="fj-buttons"><a class="all" href="index.html#scholarship">View All Scholarships</a><a class="apply" href="${esc(apply)}" ${apply!=="#"?"target='_blank' rel='noopener'":""}>Apply Online</a><a class="official" href="${esc(official)}" ${official!=="#"?"target='_blank' rel='noopener'":""}>Official Website</a></div>
    <div class="fj-grid">
      ${card("Important Dates", `<ul><li>Application Start Date : <b>${esc(p.startDate||"Not specified")}</b></li><li>Application Last Date : <b>${esc(p.lastDate||"Not specified")}</b></li></ul>`)}
      ${card("Academic Session", `<b>${esc(p.academicSession||"Not specified")}</b>`)}
      ${card("Scholarship Amount", `<b>${esc(p.amount||"Check official notification")}</b>`)}
      ${card("Income Limit", `<b>${esc(p.incomeLimit||"Check official notification")}</b>`)}
    </div>
...
function renderDetail(p){
  const normalizedType=normalizeType(field(p,["type","category"],"job"));
  if(normalizedType==="blog"){renderPublishedBlog(p);return;}
  if(normalizedType==="scholarship"){renderScholarshipDetail(p);return;}
  injectFastJobStyles();
...
async function loadPost() {
    ...
    try {
        ...
        const loadedType = normalizeType(field(post,["type","category"],"job"));
        if(loadedType === "scholarship"){
            location.replace(`scholarship-detail.html?id=${encodeURIComponent(postId)}`);
            return;
        }
        if(loadedType === "blog"){
            location.replace(`blog-detail.html?id=${encodeURIComponent(postId)}`);
            return;
        }
        renderDetail(post);
...
/* =========================================================
   RAPID JOB LIVE USER FEATURES — FIREBASE USER-WISE DATA
   Additive layer: existing post/blog/scholarship/ads code untouched.
   ========================================================= */
(function rapidJobLiveUserFeatures(){
  "use strict";
  const state={post:null,user:null,uid:null,db:null,compare:[]};
  ...
  function getPost(){
    if(!state.post){
      const t=document.querySelector('.fj-title')?.textContent?.trim()||document.title.replace(/\s*-\s*Rapid Job.*$/i,'');
      state.post={id:postId||location.pathname+location.search,title:t,url:location.href,category:postCategory||'job'};
    }
    return state.post;
  }
  function toast(message,type='ok'){
    let el=document.getElementById('rjUserToast');
    if(!el){el=document.createElement('div');el.id='rjUserToast';document.body.appendChild(el);}
    el.className='rj-user-toast show '+type;el.textContent=message;
    clearTimeout(el._t);el._t=setTimeout(()=>el.classList.remove('show'),2400);
  }
  function requireLogin(){
    if(state.user) return true;
    toast('Please login to use this feature.','warn');
    setTimeout(()=>location.href='login.html?redirect='+encodeURIComponent(location.href),650);
    return false;
  }
  function cleanKey(v){return String(v||'item').replace(/[.#$\[\]/]/g,'_').slice(0,180)}
  function userRef(section,id){return state.db.ref(`users/${state.uid}/${section}/${cleanKey(id)}`)}
  function postPayload(extra={}){
    const p=getPost();
    return {id:p.id,title:p.title,url:p.url,category:p.category,updatedAt:now(),...extra};
  }
  async function saveJob(){
    if(!requireLogin())return;
    const p=getPost(), ref=userRef('savedJobs',p.id); const snap=await ref.once('value');
    if(snap.exists()){await ref.remove();toast('Job removed from Saved Jobs.');}
    else{await ref.set(postPayload({savedAt:now()}));toast('Job saved successfully.');}
    refreshActionState();
  }
  async function markApplied(){
    if(!state.user)return;
    const p=getPost(); await userRef('appliedJobs',p.id).set(postPayload({appliedAt:now(),status:'Applied'}));
  }
  async function addRecent(){
    if(!state.user)return;
    const p=getPost(); await userRef('recentJobs',p.id).set(postPayload({viewedAt:now()}));
  }
  async function refreshActionState(){
    const btn=document.getElementById('rjSaveJobBtn'); if(!btn)return;
    if(!state.user){btn.innerHTML='<i class="fa-regular fa-bookmark"></i> Save Job';return;}
    const snap=await userRef('savedJobs',getPost().id).once('value');
    btn.classList.toggle('saved',snap.exists());
    btn.innerHTML=snap.exists()?'<i class="fa-solid fa-bookmark"></i> Saved':'<i class="fa-regular fa-bookmark"></i> Save Job';
  }
  function parseDate(v){
    if(!v)return null; const s=String(v).trim();
    const dmy=s.match(/(\d{1,2})[\/\-. ](\d{1,2}|[A-Za-z]+)[\/\-. ,](\d{4})/);
    if(dmy){const months={jan:0,feb:1,mar:2,apr:3,may:4,jun:5,jul:6,aug:7,sep:8,oct:9,nov:10,dec:11};let m=isNaN(dmy[2])?months[dmy[2].slice(0,3).toLowerCase()]:Number(dmy[2])-1;return new Date(Number(dmy[3]),m,Number(dmy[1]),23,59,59)}
    const d=new Date(s);return isNaN(d)?null:d;
  }
  function countdownText(date){
    const diff=date-Date.now(); if(diff<0)return 'Application Closed';
  ...
  function addCountdown(){
    const dates=[...document.querySelectorAll('.fj-card')].find(x=>/Important Dates/i.test(x.querySelector('h2')?.textContent||''));
    if(!dates||document.getElementById('rjDeadlineCountdown'))return;
    const text=dates.textContent; const m=text.match(/(?:last date|application last date)\s*:?\s*([^\n]+)/i); const d=parseDate(m?.[1]); if(!d)return;
    const box=document.createElement('div');box.id='rjDeadlineCountdown';box.className='rj-deadline-countdown';dates.appendChild(box);
    const tick=()=>{box.innerHTML=`<i class="fa-solid fa-clock"></i><span>${countdownText(d)}</span>`};tick();setInterval(tick,60000);
  }
  function compareList(){try{return JSON.parse(localStorage.getItem('rjCompareJobs')||'[]')}catch(e){return[]}}
  function toggleCompare(){
    let list=compareList(),p=getPost(),i=list.findIndex(x=>String(x.id)===String(p.id));
    if(i>=0){list.splice(i,1);toast('Removed from comparison.');}
    else{if(list.length>=3){toast('Maximum 3 jobs can be compared.','warn');return;}list.push(postPayload({updatedAt:Date.now()}));toast('Added to Job Compare.');}
    localStorage.setItem('rjCompareJobs',JSON.stringify(list));refreshCompareButton();
  }
  function refreshCompareButton(){const b=document.getElementById('rjCompareBtn');if(!b)return;const yes=compareList().some(x=>String(x.id)===String(getPost().id));b.classList.toggle('selected',yes);b.innerHTML=yes?'<i class="fa-solid fa-scale-balanced"></i> Added':'<i class="fa-solid fa-scale-balanced"></i> Compare';}
  function openEligibility(){
    const age=document.querySelector('.fj-card:nth-of-type(3)')?.innerText||document.body.innerText;
    const modal=document.createElement('div');modal.className='rj-feature-modal';modal.innerHTML=`<div class="rj-feature-box"><button class="rj-feature-close">×</button><h2>Smart Eligibility Check</h2><p>Enter your age to perform a basic check. Final eligibility must be verified from the official notification.</p><input id="rjEligibilityAge" type="number" min="1" max="100" placeholder="Your age"><button id="rjCheckEligibility">Check Now</button><div id="rjEligibilityResult"></div></div>`;document.body.appendChild(modal);requestAnimationFrame(()=>modal.classList.add('show'));
    const close=()=>modal.remove();modal.querySelector('.rj-feature-close').onclick=close;modal.onclick=e=>{if(e.target===modal)close()};
    modal.querySelector('#rjCheckEligibility').onclick=()=>{const a=Number(modal.querySelector('#rjEligibilityAge').value);const r=modal.querySelector('#rjEligibilityResult');if(!a){r.textContent='Please enter a valid age.';return;}const nums=[...age.matchAll(/\b(\d{1,2})\b/g)].map(x=>Number(x[1])).filter(x=>x>=10&&x<=70);const min=nums.length?Math.min(...nums):null,max=nums.length?Math.max(...nums):null;r.innerHTML=min!==null?(a>=min&&a<=max?`<b class="yes">Likely eligible by age (${min}–${max} years).</b>`:`<b class="no">Age may not match (${min}–${max} years).</b>`):'Age limits could not be read automatically. Check the official notification.';};
  }
  function openPdf(){
    const links=[...document.querySelectorAll('a')];const a=links.find(x=>/notification|pdf|download notice/i.test(x.textContent||'')&&x.href&&x.href!=='#');if(!a){toast('PDF/notification link is not available.','warn');return;}
    const modal=document.createElement('div');modal.className='rj-feature-modal pdf';modal.innerHTML=`<div class="rj-feature-box"><button class="rj-feature-close">×</button><h2>Notification Viewer</h2><iframe src="${safe(a.href)}"></iframe><a class="rj-open-new" target="_blank" rel="noopener" href="${safe(a.href)}">Open in New Tab</a></div>`;document.body.appendChild(modal);requestAnimationFrame(()=>modal.classList.add('show'));modal.querySelector('.rj-feature-close').onclick=()=>modal.remove();
    if(state.user) userRef('downloads',getPost().id).set(postPayload({downloadedAt:now(),fileUrl:a.href}));
  }
  function addActions(){
    const buttons=document.querySelector('.fj-buttons');if(!buttons||document.getElementById('rjSaveJobBtn'))return;
    const bar=document.createElement('div');bar.className='rj-user-actionbar';bar.innerHTML=`<button id="rjSaveJobBtn"><i class="fa-regular fa-bookmark"></i> Save Job</button><button id="rjCompareBtn"><i class="fa-solid fa-scale-balanced"></i> Compare</button><button id="rjEligibilityBtn"><i class="fa-solid fa-user-check"></i> Eligibility</button><button id="rjPdfViewerBtn"><i class="fa-solid fa-file-pdf"></i> PDF Viewer</button>`;buttons.insertAdjacentElement('afterend',bar);
    document.getElementById('rjSaveJobBtn').onclick=saveJob;document.getElementById('rjCompareBtn').onclick=toggleCompare;document.getElementById('rjEligibilityBtn').onclick=openEligibility;document.getElementById('rjPdfViewerBtn').onclick=openPdf;
    buttons.querySelector('.apply')?.addEventListener('click',markApplied);refreshActionState();refreshCompareButton();addCountdown();
  }
  function initAuth(){
    state.db=getDb(); if(!state.db)return;
    firebase.auth().onAuthStateChanged(u=>{state.user=u;state.uid=u?.uid||null;if(u){addRecent();refreshActionState();}});
  }
  function start(){
    initAuth();const box=document.getElementById('detailBox');if(box)new MutationObserver(()=>requestAnimationFrame(addActions)).observe(box,{childList:true,subtree:false});setTimeout(addActions,300);
    window.RapidUserFeatures={recordDownload:(id,title,url)=>{if(state.user)return userRef('downloads',id).set({id,title,url,downloadedAt:now()})},recordTest:(id,title,score,total)=>{if(state.user)return userRef('testHistory',id).set({id,title,score,total,takenAt:now()})},saveTimetable:(id,data)=>{if(state.user)return userRef('savedTimetables',id).set({...data,savedAt:now()})}};
  }
