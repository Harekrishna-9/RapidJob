...
function customSectionsHtml(p){
  ...
  return sections.sort((a,b)=>Number(a.order||50)-Number(b.order||50)).map(sec=>{
    ...
    if(style==="linkTable") return yellowLinkTable(rowsFromText(content));
    if(style==="links"){
...
function renderDetail(p){
  ...
  const hasStandard = plain(p.startDate)||plain(p.lastDate)||plain(p.fee)||plain(p.minAge)||plain(p.totalPost)||vacancy.length||yellow.length||exam.length||plain(p.selectionProcess)||plain(p.howToApply)||plain(p.salary)||plain(p.faq);
