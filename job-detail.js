...
function jobJourneyHtml(p){
  const j=p?.jobJourney;
  if(!j||j.enabled!==true||!Array.isArray(j.stages)||!j.stages.length)return "";
  const stages=j.stages.filter(s=>s&&s.label);
  if(!stages.length)return "";
  const completed=stages.filter(s=>s.status==="completed").length;
  const pct=Math.round(completed/stages.length*100);
  const current=stages.find(s=>s.status==="current")||stages.find(s=>s.status!=="completed")||stages[stages.length-1];
  const prettyDate=value=>{if(!value)return "Date Awaited";try{return new Date(value+"T00:00:00").toLocaleDateString("en-IN",{day:"2-digit",month:"short",year:"numeric"})}catch(e){return value}};
  return `<section class="rj-job-journey" aria-label="Recruitment Journey">
    <div class="rj-journey-head">
      <div><span class="rj-journey-kicker"><i class="fa-solid fa-route"></i> Live Recruitment Tracker</span><h2>Job Journey</h2><p>Notification se joining tak poori recruitment progress ek jagah.</p></div>
      <div class="rj-current-stage"><small>Current Stage</small><strong>${esc(current?.label||"Update Awaited")}</strong></div>
    </div>
    <div class="rj-journey-progress-row"><span>Recruitment Progress</span><b>${pct}%</b></div>
    <div class="rj-journey-progress"><span style="width:${pct}%"></span></div>
    <div class="rj-journey-timeline">
      ${stages.map((s,i)=>{const status=["completed","current"].includes(s.status)?s.status:"upcoming";const url=plain(s.link||"");return `<div class="rj-journey-step ${status}">
        <div class="rj-step-marker"><i class="fa-solid ${esc(s.icon||"fa-circle")}"></i></div>
        <div class="rj-step-card"><div class="rj-step-top"><span>Stage ${i+1}</span><em>${status==="completed"?"Completed":status==="current"?"Current":"Upcoming"}</em></div><h3>${esc(s.label)}</h3><p><i class="fa-regular fa-calendar"></i> ${esc(prettyDate(s.date))}</p>${url?`<a href="${esc(url)}" target="_blank" rel="noopener">Open Update <i class="fa-solid fa-arrow-up-right-from-square"></i></a>`:""}</div>
      </div>`}).join("")}
    </div>
  </section>`;
}
