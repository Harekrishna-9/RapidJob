...
function jobJourneyHtml(p){const j=p&&p.jobJourney;if(!j||!j.enabled||!Array.isArray(j.stages)||!j.stages.length)return"";const done=j.stages.filter(s=>s.status==="completed").length,progress=Math.round(done/j.stages.length*100),current=j.stages.find(s=>s.key===j.currentStage)||j.stages.find(s=>s.status==="current"),fd=v=>{if(!v)return"";try{return new Date(v+"T00:00:00").toLocaleDateString("en-IN",{day:"2-digit",month:"short",year:"numeric"})}catch(e){return v}};return `<section class="rj-job-journey"><div class="rj-journey-head"><div><span><i class="fa-solid fa-route"></i> Recruitment Tracker</span><h2>Job Journey</h2></div><div class="rj-journey-percent">${progress}%</div></div><div class="rj-journey-progress"><i style="width:${progress}%"></i></div>${current?`<div class="rj-current-stage"><i class="fa-solid fa-location-dot"></i><div><small>Current Stage</small><strong>${esc(current.label||"Recruitment Update")}</strong></div></div>`:""}<div class="rj-journey-timeline">${j.stages.map((s,i)=>{const c=`<div class="rj-stage-copy"><strong>${esc(s.label||"Stage")}</strong>${s.date?`<span>${esc(fd(s.date))}</span>`:""}</div>`;return `<article class="rj-journey-stage ${esc(s.status||"upcoming")}"><div class="rj-stage-marker"><b>${i+1}</b></div>${s.link?`<a href="${esc(s.link)}" target="_blank" rel="noopener">${c}<em>Open Update</em></a>`:`<div>${c}</div>`}</article>`}).join("")}</div></section>`}
